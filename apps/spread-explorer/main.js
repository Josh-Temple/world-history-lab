...
async function loadEvents() { return fetchJson('/data/events.json', 'events'); }
...
init().catch((error) => {
  ...
  if (panel) panel.innerHTML = `<div class="event-card">Unable to load spread data: ${error.message}</div>`;
});
